			...
			switch (cmd.split(' ')[0]) {
				...
				case "count":
					if (cmd.split(' ')[1]) {
						...
						if (queryid in youData)	msg.channel.sendMessage(`${queryid.mention2id()} has ${youData[queryid][1].toString().code()} ${youji}s.`);
				  ...
				  if (msg.channel.id === "284733446029312000" && cmd.split(' ')[1] && msg.author.id in youData) {
				    ...
				    if (new Date().getTime() - youData[msg.author.id][2] > 1800000) {
				      ...
				      if (youData[msg.author.id][1] > 0 && bet > 0 && youData[msg.author.id][1] >= bet) {
						...
						else msg.channel.sendMessage(`\u{1f3b2} ${msg.author}, please wait ${(1800000 - new Date().getTime() + youData[msg.author.id][2]).timeCounter()} until your next ${youji}s roulette.`);
				  } break;
				case "gift":
				...
				case "":
					if (msg.channel.id === "284733446029312000") {
						if (msg.author.id in youData) {
							...
							else msg.channel.sendMessage(`${msg.author}, please wait until midnight UTC, in ${(86400000 - new Date().getTime() % 86400000).timeCounter()}, for your next ${youji}.`);
						}
						else {
